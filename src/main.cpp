...
int64 nChainStartTime = 1467360834;
...
const string strMessageMagic = "lontaicoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, const CBlockIndex* pindex)
{
     ...
     if (nPoWHeight == 1)
       nSubsidy = 86000000 * COIN; //Premin: 28M
     else
       nSubsidy = 0 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    int64 nSubsidy = nCoinAge * nRewardCoinYear / 365;  //2%

    if(nHeight < YEARLY_POS_BLOCK_COUNT * 1)            //8%
      nSubsidy *= 1;
    else if(nHeight < YEARLY_POS_BLOCK_COUNT * 2)       //6%
      nSubsidy *= 1;
    else if(nHeight < YEARLY_POS_BLOCK_COUNT * 3)       //4%
      nSubsidy *= 1;
    else if(nHeight > YEARLY_POS_BLOCK_COUNT * 4)       //0%
      nSubsidy = 0;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in lontaicoin
    bool fStrictPayToScriptHash = true; // Always active in lontaicoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "lontaicoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "nothing to say.China's annual parliament meets under economic clouds. 2016-03-20";
        CTransaction txNew;
      ...
      if (true  && (block.GetHash() != hashGenesisBlock))
        ...
        assert(block.hashMerkleRoot == uint256("0xcd68dfeae838adc2a16d69da697758348e109757fbbb92523d21d5e9cb748ba0"));
...
unsigned char pchMessageStart[4] = { 0x74, 0xf5, 0x1c, 0x62 };
